...
# Load data
metro2005 = read.dta("01_Data/01_Raw/01_COMIPEMS_CURP/metro_2005/metro2005.dta")
metro2005 = setDT(metro2005)
...
# Save cutoff_school
write_dta(cutoff_school_2005, path = "01_Data/02_Created/cutoff_school_2005.dta", version = 15)
save(cutoff_school_2005, file = "01_Data/02_Created/cutoff_school_2005.RData")

# Get school list
SchoolList = sort(unique(cutoff_school_2005$school_code))

#Just to test the code that elimintates irrelevant options
#metro2005=metro2005[curp=="BESM900201HDFCNR",]
...
write_dta(metro2005, path = "01_Data/02_Created/metro2005_fixed.dta", version = 15)
save(metro2005, file = "01_Data/02_Created/metro2005_fixed.RData")
...
DFMetro2005=data.frame(metro2005)
DFMetro2005=DFMetro2005[,paste0("cutoff_opc_ed",str_pad(1:20,2,pad="0"))]
...
for(sc_temp in SchoolList){
  ...
  #metro2005[,eval(y):=NULL]
  metro2005[opc_ed01==sc_temp,eval(y):=0]
  
  for(i in 2){
    padded_i=str_pad(i, 2, pad = "0")
    index=which(metro2005[[eval(paste0("opc_ed",padded_i))]]==sc_temp)
    metro2005[index,(y):=cutoff_opc_ed01]
  }
  
  for(i in 3:20){
    padded_i=str_pad(i, 2, pad = "0")
    index=which(metro2005[[eval(paste0("opc_ed",padded_i))]]==sc_temp)
    metro2005[index,eval(y):=apply(DFMetro2005[index,paste0("cutoff_opc_ed",str_pad((1):(i-1),2,pad="0"))],1,max,na.rm=T)]
  }
}
...
metro2005_basic=metro2005[, ..ColsBasicas]

write_dta(metro2005_basic, path = "01_Data/02_Created/metro2005_MID.dta", version = 15)
save(metro2005_basic, file = "01_Data/02_Created/metro2005_MID.RData")
...
for(sc_temp in SchoolList){
  ...
  index=which(metro2005_basic[[z]]>tau_s)
  metro2005_basic[index,eval(y):=0]
  #metro2005_basic[[y]]=as.numeric(metro2005_basic[[y]])
  
  #Part2a: zero if score<=MID-delta or score>tau_s+delta 
  index=which((metro2005_basic$normalized_score<=(metro2005_basic[[z]]-delta1)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  index=which((metro2005_basic$normalized_score>(tau_s+delta1)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  
  #Part2b: 1 if score \in (MID+delta,tau_s-delta]
  index=which((metro2005_basic$normalized_score>(metro2005_basic[[z]]+delta1)) &  (metro2005_basic$normalized_score<=(tau_s-delta1)) & metro2005_basic[[z]]<tau_s  )
...
write_dta(metro2005_pdelta, path = "01_Data/02_Created/metro2005_pdelta1.dta", version = 15)
...
for(sc_temp in SchoolList){
  ...
  #Part2a: zero if score<=MID-delta or score>tau_s+delta 
  index=which((metro2005_basic$normalized_score<=(metro2005_basic[[z]]-delta2)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  index=which((metro2005_basic$normalized_score>(tau_s+delta2)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  
  #Part2b: 1 if score \in (MID+delta,tau_s-delta]
  index=which((metro2005_basic$normalized_score>(metro2005_basic[[z]]+delta2)) &  (metro2005_basic$normalized_score<=(tau_s-delta2)) & metro2005_basic[[z]]<tau_s  )
  metro2005_basic[index,eval(y):=10]
...
write_dta(metro2005_pdelta, path = "01_Data/02_Created/metro2005_pdelta2.dta", version = 15)
...
for(sc_temp in SchoolList){
  ...
  #Part2a: zero if score<=MID-delta or score>tau_s+delta 
  index=which((metro2005_basic$normalized_score<=(metro2005_basic[[z]]-delta3)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  index=which((metro2005_basic$normalized_score>(tau_s+delta3)) & metro2005_basic[[z]]<tau_s)
  metro2005_basic[index,eval(y):=0]
  
  #Part2b: 1 if score \in (MID+delta,tau_s-delta]
  index=which((metro2005_basic$normalized_score>(metro2005_basic[[z]]+delta3)) &  (metro2005_basic$normalized_score<=(tau_s-delta3)) & metro2005_basic[[z]]<tau_s  )
  metro2005_basic[index,eval(y):=10]
...
write_dta(metro2005_pdelta, path = "01_Data/02_Created/metro2005_pdelta3.dta", version = 15)
