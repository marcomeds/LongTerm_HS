  ...
  \begin{itemize}
    \vfill\item Maybe they have effects on labor market outcomes?
     ...
     \vfill\item Peer composition does changes \tiny{\citep{ElliteIllusion,DobbieFryer2014,ParentsValueEffectiveness}}
       \end{itemize}
    \vfill\item Current RD estimates may not reflect treatment effects for most applicants
  \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{The 361 public schools in Mexico City are managed by nine entities}
  \begin{itemize}
...
% \begin{frame}{High-school subsystems map}
%     \centering
%     \includegraphics[width=0.65\textwidth]{04_Figures/comipems_subsystem_map_ggmap.pdf}
% \end{frame}

\begin{frame}{High-school types map}
    \centering
...
\begin{frame}{Outcome data}
  ...
  \vfill\item Taking the exam is a good proxy for graduating (except for UNAM students)
    ...
    \vfill\item Data from The National Electoral Institute (INE) for 2012, 2015, and \textbf{2018}
      \begin{itemize}
    \vfill\item If voted 
...
\vfill\item Control for the local propensity score at each school
...
\alt<5-7>{
    \begin{itemize}
        ...
        \item<7-> Negative ITT effect of being marginally assigned to an IPN high school around the MID.
    \end{itemize}}
    {\begin{subfigure}{\textwidth}
        \centering
...
\begin{itemize}
\vfill \item Treatment effect heterogeneity matters --- but need to fix the counterfactual 
...
\vfill \item \normalsize Estimate ``value added'' of different schools on different outcomes (against a fix counterfactual), for different children
